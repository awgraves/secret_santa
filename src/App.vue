<template>
  <div id="app">
    <TopBar></TopBar>
    <div id="cards-area">
      <div v-if="!users.length" id="start-text-container">
        <div class="simulator-text simulator-text--flicker">Click 'Participants' to Start!</div>
      </div>
      <UserCard v-for="(user, indexID) in users"
                :user="user"
                :key="'user-' + indexID">
      </UserCard>
      <ParticipantsMenu v-show="menuOpened"/>
    </div>
  </div>
</template>
...
import ParticipantsMenu from './components/ParticipantsMenu.vue'
